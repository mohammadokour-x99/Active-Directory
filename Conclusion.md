...
<node><rich_text>Responder.py : returns NTLMv2 hash

Ntlmrelayx.py : relaying captured hash from responder.py to the target machine/</rich_text><rich_text scale="small">s</rich_text><rich_text> , which will give u a lot of things such as : 
	- local sam hases 
...
</rich_text><rich_text foreground="#f6d32d">Kerberoasting attack: </rich_text><rich_text> is a post-compromise Active Directory attack where a domain user requests a Kerberos service ticket (TGS) for a service account and then attempts to crack the ticket offline to recover the service account's password.

</rich_text><rich_text foreground="#f6d32d">GPP attack :</rich_text><rich_text> is an Active Directory attack that abuses passwords stored in Group Policy Preferences.
